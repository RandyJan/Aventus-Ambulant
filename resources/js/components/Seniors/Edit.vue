                    ...
                    <div class="mx-auto mb-14 mt-5 p-5 pt-0 max-w-lg">
                        ...
                        <div class="mt-5">
                            ...
                            <button
                                ...
                                :disabled="!can_update"
                                :class="{
                                    'cursor-not-allowed opacity-50' : !can_update,
                                    'hover:bg-sky-700' : can_update,
                                }"
                                class="mt-4 tracking-wider w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-sky-600  focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
                                Update
                            </button>
...
export default {
    ...
    methods: {
        ...
        submit(){
            ...
            confirm.fire({
                text: 'This action will update the senior data.',
                // confirmButtonText: 'Proceed to update Table number.',
            }).then( (result) => {
                if(result.isConfirmed){
                    axios.patch(`/orderslip-seniors/${ this.payload.ID }`, {
                        sc_id: this.id,
                        sc_name: this.name,
    ...
    created(){
        this.id = this.payload.SC_ID;
        this.name = this.payload.SC_NAME;
        this.address = this.payload.SC_ADDRESS;
    }
}
</script>
